...
function broadcast(channel, payload) {
  for (const win of [mainWin, overlayWin, telemetryWin]) {
...
/* ---------------- отдельный overlay телеметрии ---------------- */

function applyTelemetryFlags() {
  if (!telemetryWin || telemetryWin.isDestroyed()) return;
  telemetryWin.setOpacity(1);
  telemetryWin.setIgnoreMouseEvents(!!settings.telemetry.clickThrough, { forward: true });
  telemetryWin.setMovable(!settings.telemetry.locked && !settings.telemetry.clickThrough);
  telemetryWin.setResizable(!settings.telemetry.locked && !settings.telemetry.clickThrough);
  telemetryWin.setAlwaysOnTop(true, "screen-saver");
  telemetryWin.webContents.send("sp:telemetry-config", settings.telemetry);
  if (mainWin && !mainWin.isDestroyed()) mainWin.webContents.send("sp:telemetry-config", settings.telemetry);
}

function createTelemetryWindow() {
  const area = screen.getPrimaryDisplay().workAreaSize;
  const saved = settings.telemetryBounds || {};
  telemetryWin = new BrowserWindow({
    width: saved.width || 480,
    height: saved.height || 108,
    x: Number.isInteger(saved.x) ? saved.x : area.width - 510,
    y: Number.isInteger(saved.y) ? saved.y : area.height - 150,
    minWidth: 150,
    minHeight: 70,
    frame: false,
    transparent: true,
  ...
  telemetryWin.setAlwaysOnTop(true, "screen-saver");
  telemetryWin.setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true });
  telemetryWin.loadFile(RENDERER, { hash: "/telemetry" });
  telemetryWin.on("closed", () => (telemetryWin = null));
  telemetryWin.webContents.on("did-finish-load", () => {
    applyTelemetryFlags();
    if (telemetryService) telemetryWin.webContents.send("sp:telemetry", telemetryService.snapshot);
  });

  const remember = () => {
    if (!telemetryWin || telemetryWin.isDestroyed()) return;
    settings.telemetryBounds = telemetryWin.getBounds();
    persist();
  };
  telemetryWin.on("moved", remember);
  telemetryWin.on("resized", remember);
  if (settings.telemetry.enabled) telemetryWin.showInactive();
}

function toggleTelemetry(force) {
  if (!telemetryWin || telemetryWin.isDestroyed()) createTelemetryWindow();
  const show = force !== undefined ? force : !telemetryWin.isVisible();
  if (show) telemetryWin.showInactive();
  else telemetryWin.hide();
  settings.telemetry.enabled = show;
  persist();
  applyTelemetryFlags();
}
...
function trayMenu() {
  return Menu.buildFromTemplate([
    ...
    { label: "Телеметрия вкл/выкл", click: () => toggleTelemetry() },
...
function registerHotkeys() {
  ...
  const local = {
    ...
    "telemetry:toggle": () => toggleTelemetry(),
    "window:toggle": () => {
      if (!mainWin) return;
...
ipcMain.handle("telemetry:get", () => telemetryService?.snapshot || null);
ipcMain.handle("telemetry:config", () => settings.telemetry);
ipcMain.on("telemetry:set", (_e, cfg) => {
  const wasEnabled = settings.telemetry.enabled;
  settings.telemetry = { ...settings.telemetry, ...cfg };
  persist();
  if (settings.telemetry.enabled !== wasEnabled) toggleTelemetry(settings.telemetry.enabled);
  else applyTelemetryFlags();
});
...
app.on("will-quit", () => {
  ...
  try { if (telemetryService) telemetryService.stop(); } catch { /* noop */ }
...
app.whenReady().then(async () => {
  ...
  telemetryService = new TelemetryService({
    onUpdate: (stats) => emit("sp:telemetry", stats),
  });
  telemetryService.start();
  if (settings.telemetry.enabled) createTelemetryWindow();
