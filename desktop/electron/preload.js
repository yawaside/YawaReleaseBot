...
const mode = location.hash.includes("telemetry") ? "telemetry" : location.hash.includes("overlay") ? "overlay" : "app";
...
contextBridge.exposeInMainWorld("sp", {
  ...
  telemetry: {
    ...
    config: () => ipcRenderer.invoke("telemetry:config"),
    set: (cfg) => ipcRenderer.send("telemetry:set", cfg),
    onStats: (cb) => ipcRenderer.on("sp:telemetry", (_e, stats) => cb(stats)),
    onChange: (cb) => ipcRenderer.on("sp:telemetry-config", (_e, cfg) => cb(cfg)),
  },
