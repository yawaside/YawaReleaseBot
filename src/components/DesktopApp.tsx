...
import {
  Activity, AudioWaveform, Ban, Gamepad2, Info, Keyboard, LayoutList, Menu, Minus,
...
import TelemetryPanel from "./app/TelemetryPanel";
...
import { DEFAULT_TELEMETRY, EMPTY_TELEMETRY } from "../lib/telemetry";
...
type TabId = "voice" | "filters" | "chatview" | "widget" | "overlay" | "telemetry" | "hotkeys" | "interface" | "about";
...
export default function DesktopApp() {
  ...
  const [telemetry, setTelemetryState] = useState<TelemetryConfig>(DEFAULT_TELEMETRY);
  const [telemetryStats, setTelemetryStats] = useState<TelemetrySnapshot>(EMPTY_TELEMETRY);
  useEffect(() => {
    if (!sp?.telemetry) {
      let tick = 0;
      const makeDemo = (): TelemetrySnapshot => ({
        updatedAt: Date.now(),
        cpu: { usage: 31 + ((tick * 7) % 18), temperature: 58 + ((tick * 3) % 7) },
        gpu: { name: "NVIDIA GeForce RTX", usage: 54 + ((tick * 9) % 28), temperature: 64 + ((tick * 2) % 8), discrete: true },
      });
      setTelemetryStats(makeDemo());
      const iv = window.setInterval(() => { tick += 1; setTelemetryStats(makeDemo()); }, 2000);
      return () => window.clearInterval(iv);
    }
    sp.telemetry.config().then((cfg) => cfg && setTelemetryState({ ...DEFAULT_TELEMETRY, ...cfg }));
    sp.telemetry.get().then((stats) => stats && setTelemetryStats(stats));
    sp.telemetry.onChange((cfg) => setTelemetryState((cur) => ({ ...cur, ...cfg })));
    sp.telemetry.onStats((stats) => setTelemetryStats(stats));
  }, [sp]);

  const patchTelemetry = useCallback(
    (patch: Partial<TelemetryConfig>) => {
      setTelemetryState((cur) => {
        const next = { ...cur, ...patch };
        if (sp?.telemetry) sp.telemetry.set(patch);
        else {
          try { localStorage.setItem("yawa:telemetry", JSON.stringify(next)); } catch { /* noop */ }
        }
        return next;
      });
      markSaved();
